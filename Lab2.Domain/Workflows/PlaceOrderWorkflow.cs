...
using static Lab2.Domain.Models.OrderPlacedEvent;
   ...
   public class PlaceOrderWorkflow
{
    ...
    public PlaceOrderWorkflow(IProductsRepository productsRepository, IOrderLineRepository orderlineRepository, IOrderHeaderRepository orderHeaderRepository, ILogger<PlaceOrderWorkflow> logger)
    {
    ...
    public async Task<IOrderPlacedEvent> ExecuteAsync(PlaceOrderCommand command)
    {
        try
        {
            ...
            return OrderPlacedEvent.ToEvent(order);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "An error occurred while creating the order");
            return new OrderPlacedEvent.OrderPlacingFailedEvent("Unexpected error");
        }
    }

    private IOrder ExecuteBusinessLogic(PlaceOrderCommand command, List<ProductId> existingProducts, List<CalculatedOrderLine> existingOrders)
    {
